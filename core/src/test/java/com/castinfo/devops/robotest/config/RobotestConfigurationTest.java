...
public class RobotestConfigurationTest {
    ...
    public void testLoadBasicConfig() {
        ...
        Assert.assertTrue(basicConfig.getReportFilePath()
                                     .equals(System.getProperty("java.io.tmpdir")));
        Assert.assertTrue(basicConfig.getEnv()
                                     .equals("local"));
        Assert.assertTrue(basicConfig.getBrowser()
                                     .getBrowserName()
                                     .equals(SeleniumBrowser.CHROME.name()));
        Assert.assertTrue(basicConfig.getBrowser()
                                     .getConsoleLogLevel()
                                     .equals("WARNING"));
        Assert.assertTrue(!basicConfig.getHttp()
                                      .getGeneralTimeout()
                                      .isEmpty());
        Assert.assertTrue(basicConfig.getDocker()
                                     .getHost()
                                     .equals("tcp://192.168.99.100:2376"));
        Assert.assertTrue(basicConfig.getDocker()
                                     .getCertsPath()
        ...
        Assert.assertTrue(basicConfig.getDocker()
                                     .getExposeDebugPort()
                                     .equals("5900"));
        Assert.assertTrue(basicConfig.getBrowserStack()
                                     .getDevice()
                                     .equals("IPHONE X"));
        Assert.assertTrue(basicConfig.getBrowserStack()
                                     .getPlatform()
        ...
        Assert.assertTrue(basicConfig.toString()
                                     .length() > 0);
    ...
    public void testLoadUserConfigProperties() throws RobotestException {
        ...
        Mockito.when(cfgAnnot.resource())
               .thenReturn("classpath://testeable-local.properties");
        Mockito.when(cfgAnnot.type())
               .thenReturn(Properties.class);
        RobotestConfig[] cfgAnnotArray = { cfgAnnot };
        RobotestSuite suiteAnnot = Mockito.mock(RobotestSuite.class);
        Mockito.when(suiteAnnot.configElements())
               .thenReturn(cfgAnnotArray);
        rCfg.loadAnnotationScopeConfig(suiteAnnot.configElements(),
                                       suiteAnnot);
        List<ConfigEntry> lCfgEntries = rCfg.toReportConfigEntries(suiteAnnot);
        Assert.assertTrue(lCfgEntries.size() == 1);
        Assert.assertTrue(lCfgEntries.get(0)
                                     .getValue() instanceof Properties);
        Assert.assertTrue(((Properties) lCfgEntries.get(0)
                                                   .getValue()).getProperty("TESTEABLE_KEY")
    ...
    public void testLoadUserConfigJackson() throws RobotestException {
        ...
        RobotestConfig cfgAnnot = Mockito.mock(RobotestConfig.class);
        Mockito.when(cfgAnnot.resource())
               .thenReturn("classpath://testeable-docker.json");
        Mockito.when(cfgAnnot.type())
               .thenReturn(DockerConfig.class);
        RobotestConfig[] cfgAnnotArray = { cfgAnnot };
        RobotestSuite suiteAnnot = Mockito.mock(RobotestSuite.class);
        Mockito.when(suiteAnnot.configElements())
               .thenReturn(cfgAnnotArray);
        ...
        Assert.assertTrue(lCfgEntries.size() == 1);
        Assert.assertTrue(lCfgEntries.get(0)
                                     .getValue() instanceof DockerConfig);
        DockerConfig docker = (DockerConfig) lCfgEntries.get(0)
                                                        .getValue();
        Assert.assertTrue(docker.getHost()
                                .equals("TEST_HOST"));
        Assert.assertTrue(docker.toString()
                                .length() > 0);
    ...
    public void testLoadUserConfigJaxb() throws RobotestException {
        ...
        RobotestConfig cfgAnnot = Mockito.mock(RobotestConfig.class);
        Mockito.when(cfgAnnot.resource())
               .thenReturn("classpath://testeable-jaxb.xml");
        Mockito.when(cfgAnnot.type())
               .thenReturn(Jaxb.class);
        RobotestConfig[] cfgAnnotArray = { cfgAnnot };
        RobotestSuite suiteAnnot = Mockito.mock(RobotestSuite.class);
        Mockito.when(suiteAnnot.configElements())
               .thenReturn(cfgAnnotArray);
        ...
        Assert.assertTrue(lCfgEntries.size() == 1);
        Assert.assertTrue(lCfgEntries.get(0)
                                     .getValue() instanceof Jaxb);
        Jaxb xml = (Jaxb) lCfgEntries.get(0)
                                     .getValue();
        Assert.assertTrue(xml.getValue()
                             .equals("TEST"));
    }
