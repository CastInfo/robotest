...
public class DockerFarmBuilderTest {
    ...
    public void testContainerCallback() throws IOException {
        try (DockerContainerCallback callback = new DockerContainerCallback("Selenium Server is up and running")) {
            ...
            Mockito.when(frame.toString())
                   .thenReturn("Selenium Server is up and running");
    ...
    public void testFarmBuilder() throws RobotestException {
        ...
        Mockito.when(docker.infoCmd())
               .thenReturn(infoCmd);
        Info info = Mockito.mock(Info.class);
        Mockito.when(infoCmd.exec())
               .thenReturn(info);

        try {
            farm.connectDockerClient();
        } catch (RobotestException e) {
            Assert.assertEquals("DOCKER CONN IS REQUIRED, REVISE BASE CONFIG",
                                e.getMessage());
        }
        ...
        Mockito.when(inspect.exec())
               .thenReturn(inspectRes);
        Mockito.when(docker.inspectContainerCmd(ArgumentMatchers.anyString()))
               .thenReturn(inspect);
        NetworkSettings netsettings = Mockito.mock(NetworkSettings.class);
        Mockito.when(inspectRes.getNetworkSettings())
               .thenReturn(netsettings);
        Ports ports = Mockito.mock(Ports.class);
        Mockito.when(netsettings.getPorts())
               .thenReturn(ports);
        Map<ExposedPort, Binding[]> bindings = new HashMap<>();
        Binding[] bind = { new Binding("",
                                       "4444") };
        bindings.put(ExposedPort.tcp(Integer.parseInt(cfgBrowser.getExposePort())),
                     bind);
        Mockito.when(ports.getBindings())
               .thenReturn(bindings);

        cfgBasic.setHost("tcp://192.168.99.100:2376");
        cfgBasic.setHub("192.168.99.100");
        farm.resolveBrowserHub(cfgBrowser);
        Assert.assertEquals("http://192.168.99.100:4444/wd/hub",
                            cfgBrowser.getHub());

        try {
            farm.resolveBrowserHub(cfgBrowser);
        } catch (RobotestException e) {
            Assert.assertEquals("DOCKER UNIX SOCKET CONN NEED DOCKER PUBLIC HOST CONFIG",
                                e.getMessage());
        }
        ...
        try {
            ...
            Assert.assertTrue(e.getMessage()
                               .startsWith("DOCKER CONN URI NOT VALID, REVISE CFG"));
        }
