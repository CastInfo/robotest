...
public class ReporterTest {
    ...
    public void testSuiteListReport() throws IOException, RobotestException {
        File suiteListFile = File.createTempFile("test",
                                                 "json");
        ...
        suiteListReport.appendToSuiteListReport("report-file-1.json");
        Assert.assertTrue(suiteListReport.getSuiteListReportContent()
                                         .getSuites()
                                         .size() == 2);
    }
    ...
    public void testSuiteReport() throws IOException, RobotestException {
        File reportFile = File.createTempFile("test",
                                              "json");
        ...
        cfg.get(0)
           .setId("TEST_CFG_KEY");
        cfg.get(0)
           .setValue("TEST_CFG_VALUE");
        RobotestConfig cfgAnnot = Mockito.mock(RobotestConfig.class);
        Mockito.when(cfgAnnot.key())
               .thenReturn("CFG_KEY");
        Mockito.when(cfgAnnot.type())
               .thenReturn(String.class);
        Mockito.when(cfgAnnot.resource())
               .thenReturn("CFG_RESOURCE");
        RobotestSuite suiteAnnot = Mockito.mock(RobotestSuite.class);
        Mockito.when(suiteAnnot.tag())
               .thenReturn("SUITE_TAG");
        Mockito.when(suiteAnnot.description())
               .thenReturn("SUITE_DESC");
        Mockito.when(suiteAnnot.configElements())
               .thenReturn(new RobotestConfig[] { cfgAnnot });
        RobotestCase caseAnnot = Mockito.mock(RobotestCase.class);
        Mockito.when(caseAnnot.tag())
               .thenReturn("CASE_TAG");
        Mockito.when(caseAnnot.description())
               .thenReturn("CASE_DESC");
        Mockito.when(caseAnnot.configElements())
               .thenReturn(new RobotestConfig[] { cfgAnnot });
        RobotestStep stepAnnot = Mockito.mock(RobotestStep.class);
        Mockito.when(stepAnnot.tag())
               .thenReturn("STEP_TAG");
        Mockito.when(stepAnnot.description())
               .thenReturn("STEP_DESC");
        Mockito.when(stepAnnot.capturePageSourceAtEndStep())
               .thenReturn(true);
        Mockito.when(stepAnnot.captureConsoleErrorLogsAtEndStep())
               .thenReturn(true);
        Mockito.when(stepAnnot.captureScreenShootAtEndStep())
               .thenReturn(true);
        try (JsonGenerator jGenerator = jfactory.createGenerator(reportFile,
                                                                 JsonEncoding.UTF8)) {
            SuiteReport sr = new SuiteReport(0,
                                             reportFile);
            sr.initSuite(suiteAnnot.tag(),
                         suiteAnnot.description(),
                         1,
                         cfg);
            ValidationEntry val = ValidationEntry.buildError()
                                                 .withMessage("VALIDATION_ENTRY");
            sr.addSuiteValidationEntry(val);
            sr.initCase(caseAnnot.tag(),
                        caseAnnot.description(),
                        3,
                        cfg);
            sr.addCaseValidationEntry(caseAnnot.tag(),
                                      val);
            sr.initStep(caseAnnot.tag(),
                        stepAnnot.tag(),
                        stepAnnot.description(),
                        cfg,
                        5);
            sr.addStepValidationEntry(caseAnnot.tag(),
                                      stepAnnot.tag(),
                                      val);
            sr.endStep(caseAnnot.tag(),
                       stepAnnot.tag(),
                       StepStatus.INFO,
                       6);
            sr.endCase(caseAnnot.tag(),
                       4);
            sr.endSuite(2);
        } finally {
            ...
            Assert.assertTrue(cfgReaded.get("order")
                                       .asInt() == 0);
            Assert.assertTrue(cfgReaded.get("suite")
                                       .asText()
                                       .equals("SUITE_TAG"));
            Assert.assertTrue(cfgReaded.get("description")
                                       .asText()
                                       .equals("SUITE_DESC"));
            Assert.assertTrue(cfgReaded.get("initMillis")
                                       .asInt() == 1);
            Assert.assertTrue(cfgReaded.get("endMillis")
                                       .asInt() == 2);
            Assert.assertTrue(cfgReaded.get("config")
                                       .get(0)
                                       .get("TEST_CFG_KEY")
                                       .asText()
                                       .equals("TEST_CFG_VALUE"));
    ...
    public void testConfigReport() throws IOException, RobotestException {
        ...
        props.setProperty("TEST_PROP_KEY",
                          "TEST_PROP_VALUE");
    ...
    public void testStepAndValidationsReport() throws IOException, RobotestException {
        ...
        try (JsonGenerator jGenerator = jfactory.createGenerator(reportFile,
            ...
            Assert.assertTrue(cfgReaded.get("order")
                                       .asInt() == 0);
            Assert.assertTrue(cfgReaded.get("step")
                                       .asText()
                                       .equals("STEP_TAG"));
            Assert.assertTrue(cfgReaded.get("description")
                                       .asText()
            ...
            Assert.assertTrue(cfgReaded.get("endMillis")
                                       .asInt() == 2);
            Assert.assertTrue(cfgReaded.get("status")
                                       .asText()
                                       .equals("INFO"));
            Assert.assertTrue(cfgReaded.get("config")
                                       .get(0)
    ...
    public void forzeIOExceptions() throws IOException, RobotestException {
        ...
        try (JsonGenerator jGenerator = Mockito.mock(JsonGenerator.class)) {
            Mockito.doThrow(new IOException("CRASH!"))
                   .when(jGenerator)
                   .writeStartArray();
            ConfigReport cfgR = new ConfigReport(cfg);
            cfgR.writeConfig(jGenerator,
